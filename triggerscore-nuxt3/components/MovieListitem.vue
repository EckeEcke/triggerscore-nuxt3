<template>
    <NuxtLink :to="`movie/${movie.id}`" tag="div" class="movie-highlight-item transition-300 w-full h-44 bg-transparent sm:bg-gradient-to-r from-gray-950 to-gray-800 md:hover:from-gray-900 text-white sm:border border-t border-b sm:border-r-0 border-gray-950 border-opacity- md:border-0 shadow-md flex relative md:rounded container-xl cursor-pointer">
...
const overview = computed(() => props.movie.overview.length > 100 ? props.movie.overview.substring(0, 110) + "..." : props.movie.overview)
