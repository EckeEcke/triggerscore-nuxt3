...
const mostCommentedMovie = computed(() =>
	store.triggerscores.length > 0
		? store.triggerscores.reduce(function (a, b) {
				const filteredArrayA = a.comments
					? a.comments.filter((comment: string) => {
							return comment.length > 3
						})
					: []

				const filteredArrayB = b.comments
...
.stats-page {
	@apply bg-transparent text-white py-6 md:pt-4 md:pb-12 md:px-0 text-left text-sm mx-auto md:rounded flex flex-col gap-x-12;
}

.swiper {
  @apply w-full sm:max-w-xl lg:max-w-none sm:flex-wrap px-4 sm:px-0 pb-2 flex sm:justify-center overflow-auto sm:mx-auto;
}
</style>
