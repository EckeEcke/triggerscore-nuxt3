  ...
  try {
    ...
    try {
      for (const locale of LOCALES_TO_FETCH) {
        ...
        if (!response.ok) {
          console.error(
            `  -> Failed to fetch locale '${locale}' for movie ${movieID}. Status: ${response.status}`,
          );
        } else {
          ...
          console.log(
            `  -> Synced locale '${locale}' for movie: ${apiData.title}`,
          );
        }
      ...
      console.error(
        `An error occurred during the movie data sync for ID ${movieID}:`,
        syncError,
      );
    }
