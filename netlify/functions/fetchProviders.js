...
export const handler = async (event) => {
  ...
  try {
    ...
    const providerDataPromises = movieIds.map((id) =>
      limiter.schedule(() =>
        fetch(
          ...
          if (!response.ok) {
            throw new Error(
              `Error fetching provider data: ${response.statusText}`,
            );
          }
    ...
    providerData.forEach((entry) => {
      if (
        !entry.results[providerRegion] ||
        !entry.results[providerRegion].flatrate
      )
        return;
      if (
      ...
      if (
        entry.results[providerRegion].flatrate.some(
          (provider) => provider.provider_name === "Amazon Prime Video",
        )
      ) {
        prime.push(entry.id);
      }
