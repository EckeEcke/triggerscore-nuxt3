...
export const handler = async (event) => {
  ...
  try {
    ...
    const top10Sexism = calculatedScores
      .sort((a, b) => b.rating_sexism - a.rating_sexism)
      .slice(0, 10);
    const top10Racism = calculatedScores
      .sort((a, b) => b.rating_racism - a.rating_racism)
      .slice(0, 10);
    const top10Others = calculatedScores
      .sort((a, b) => b.rating_others - a.rating_others)
      .slice(0, 10);
    const top10Cringe = calculatedScores
      .sort((a, b) => b.rating_cringe - a.rating_cringe)
      .slice(0, 10);
    ...
    calculatedScores.forEach((score) => {
      allScoresTotal += score.rating_total;
    });
    calculatedScores.forEach((score) => {
      allScoresSexism += score.rating_sexism;
    });
    calculatedScores.forEach((score) => {
      allScoresRacism += score.rating_racism;
    });
    calculatedScores.forEach((score) => {
      allScoresOthers += score.rating_others;
    });
    calculatedScores.forEach((score) => {
      allScoresCringe += score.rating_cringe;
    });
    ...
    const recentRatings = await database
      .collection("scores")
      .find()
      .sort({ createdAt: -1 })
      .limit(6)
