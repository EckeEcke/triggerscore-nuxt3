<template>
  <NuxtLink
    ...
    tag="div"
    class="movie-highlight-item w-full h-44 bg-white border border-gray-300 shadow-md flex relative rounded-lg transform transition duration-300 container-xl cursor-pointer max-w-md"
  >
    ...
    <div class="w-full p-3">
      <div
        class="absolute top-3 right-3 h-12 w-12 text-white rounded-lg font-semibold"
        :class="{
      ...
      <article class="text-left relative w-full h-full">
        <h3
          ...
          class="text-base w-4/5 h-12 mb-1 pr-3 font-semibold overflow-hidden"
        >
          {{ movie.title }}
        </h3>
        <h3
        ...
        <p class="text-xs w-4/5">
          {{ $t("general.released") }}: {{ movie.release_date.substring(0, 4) }}
        </p>
        <p
          v-if="overview.length > 2"
          class="text-xs absolute top-1/2 italic py-3"
        >
          "{{ overview }}"
        </p>
        <p v-else class="text-xs absolute top-1/2 italic py-3">
...
const overview = computed(() =>
  props.movie.overview.length > 90
