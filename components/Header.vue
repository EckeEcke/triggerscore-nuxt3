<template>
    <header class="w-full h-auto bg-red-950 bg-opacity-95 z-30 shadow-md flex flex-col fixed top-0">
        <section class="container mx-auto h-full p-4 xl:w-10/12 flex justify-between">
          <div class="flex" style="transform: translateY(-3px)">
            <NuxtLink to="/" tag="h1" class="leading-none text-xl md:leading-6 md:text-2xl self-center font-semibold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-200 cursor-pointer">
        ...
        >
          ...
          <nav v-if="showNav" key="sidebar" class="w-80 bg-gray-900 bg-opacity-80 backdrop-blur h-screen fixed top-0 right-0 shadow-lg md:hidden z-40">
            ...
            <div class="">
              <NuxtLink to="/" tag="div" class="text-2xl md:text-2xl self-center font-semibold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-200 cursor-pointer pb-12" @click="showNav = false">
              ...
              <div class="container flex flex-col md:flex-row mx-auto md:px-4 xl:w-10/12 absolute top-1/3 left-1/2 transform -translate-x-1/2 h-32">
                <div class="flex mx-auto mt-6 p-4 sm:px-0 h-24 self-center w-full" style="max-width: 40rem">
                  <div class="rounded-lg flex w-full justify-start">
                    ...
                    <input type="text" v-model="store.searchInput" v-on:keyup.enter="searchMovie" @input="resetSearchResults" class="lg:text-xl px-4 w-full outline-none transition" ref="searchHeader" :placeholder="$t('header.searchPlaceholder')">
